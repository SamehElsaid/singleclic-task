...
function ProductsView({ data, title, loading }) {
    return (
        <div className="container py-24" id={title.toLowerCase()}>
            <h2 className="text-2xl || font-semibold || w-fit || pb-1 || group || mx-auto || text-center || mb-5 || relative">
                {title}
                <span className="absolute || w-[50%] || h-[2px] || bg-appColor || left-1/2 || -translate-x-1/2 || bottom-0 || transition-all || duration-300 || group-hover:w-[100%]">
                </span>
            </h2>
