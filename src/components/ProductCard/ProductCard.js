...
function ProductCard({ title, price, description, image, rating, id }) {
  ...
  return (
    <div className="relative ||  h-full">
      ...
      <Link
        to={`/product/${title.replaceAll(" ", "-").replaceAll("/", "-")}-${id}`}
        className="border h-full  || border-gray-200 || rounded-md || overflow-hidden || flex || flex-col || hover:shadow-lg || hover:cursor-pointer || transition-all || hover:translate-y-[-5px] || duration-300"
      >
        
        <LazyLoadImage
          alt={title}
          className="w-full || h-[250px] || object-cover "
          placeholders={<div className="w-full h-[250px] animate-pulse bg-gray-200"></div>}
          effect="blur"
          wrapperProps={{
        ...
        <div className="px-3 flex-1 || flex || flex-col ">
          <h2 className="text-lg || font-semibold || mt-2 || flex-1 overLapP">
            {title}
          </h2>
          <p className="text-sm || text-gray-500 || mt-1 || overLapP">
            {description}
          </p>
          <p className="text-lg || font-semibold || mt-2 || text-end">
            {price}$
          </p>
        </div>
      </Link>
